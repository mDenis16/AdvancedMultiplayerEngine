...
#pragma pack(push, 8)
    ...
    public:
        ...
        atArray(const atArray& right)
        {
            ...
            m_data = (T*)GetAllocator()->allocate(m_size * sizeof(T), 16, 0);
            std::uninitialized_copy(right.m_data, right.m_data + right.m_count, m_data);
        }

        atArray(int capacity)
        {
            m_data = (T*)GetAllocator()->allocate(capacity * sizeof(T), 16, 0);
            m_count = 0;
            m_size = capacity;
        }
        ...
        void append(atArray<T> array_value)
        {
           /* auto value_array_size = array_value.size();
            ...
            if ((value_array_size + m_count) > std::numeric_limits<uint16_t>::max())
                std::cout << "RAGE atArray::append was given too large of an atArray to append";
        ...
        void append(std::vector<T> value_array)
        {
            ...
            if ((value_array_size + m_count) > std::numeric_limits<uint16_t>::max())
                std::cout << "RAGE atArray::append was given too large of a vector to append";
        ...
        void append(const std::initializer_list<T> value_array)
        {
            ...
            if ((value_array_size + m_count) > std::numeric_limits<uint16_t>::max())
                std::cout << "RAGE atArray::append was given too large of a list to append";
        ...
        void expand(uint16_t newSize)
        {
            ...
            T* newOffset = (T*)GetAllocator()->allocate(newSize * sizeof(T), 16, 0);
