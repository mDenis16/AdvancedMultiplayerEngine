...
using GameVehicle = unsigned int;
using GameHash = unsigned int;
using GameCam = unsigned int;
...
public:
	...
	PCVector3(float x, float y, float z) :
		x(x), y(y), z(z)
	{}
public:
	float x{};
...
struct CVector3
{
	...
	static CVector3 FromM128(__m128 in) { return CVector3(in.m128_f32[0], in.m128_f32[1], in.m128_f32[2]); }
	inline CVector3 operator+(CVector3 in) const
	{
		return CVector3(
			x + in.x,
			y + in.y,
...
public:
	...
	CVector3 m_fps_angles; //0x0040
	char pad_004C[20]; //0x004C
	CVector3 m_cam_pos; //0x0060 // for fps and tps
	char pad_006C[212]; //0x006C
	CVector3 m_cam_pos2; //0x0140
	char pad_014C[644]; //0x014C
	CVector3 m_tps_angles; //0x03D0
	char pad_03DC[20]; //0x03DC
	CVector3 m_fps_cam_pos; //0x03F0
...
public:
	...
	void WritePos(CVector3 Pos) {
		*(CVector3*)(((DWORD64)this + 0x50)) = Pos;
	}
};
...
public:
	...
	CPed* get_ped(const unsigned int& index) {
		return (index <= max_peds && IsValidPtr(ped_list->peds[index].gamePed) ? &ped_list->peds[index] : nullptr);
	}
};
...
public:
	...
	CVector3 v3CentreOfMassOffset; //0x0020 
	char pad_0x002C[0x4]; //0x002C
	CVector3 v3InertiaMult; //0x0030 
...
public:
	...
	CVector3 RIGHT_HAND;
	CVector3 RIGHT_FOREARM;
	CVector3 RIGHT_UPPER_ARM;
	CVector3 RIGHT_CLAVICLE;

	CVector3 LEFT_HAND;
	CVector3 LEFT_FOREARM;
	CVector3 LEFT_UPPER_ARM;
	CVector3 LEFT_CLAVICLE;
...
public:
	...
	CVector3 Aiming_at_Cord; //0x01A0
	char pad_01AC[4]; //0x01AC
	CVector3 Last_Impact_Cord; //0x01B0
